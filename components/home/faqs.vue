  ...
  <div class="hidden lg:flex">
    <div class="w-96 bg-white h-fit">
      <button
        ...
        :key="faq.id"
        :class="[
          'flex gap-2 px-2 py-3 no-wrap hover:bg-primary-100 w-full mb-1 transition-colors',
          { 'bg-primary-100': active === faq.id },
        ]"
        @click="setActive(faq.id)"
      >
        <span
          :class="[
            'w-3 h-3 mt-1.5 rounded-full',
            active === faq.id ? 'bg-primary' : 'bg-primary-100',
          ]"
        ></span>
    ...
    <div
      class="-ml-12 w-96 rounded-xl bg-primary-100 -z-[1] -mt-8 -mb-4 py-8 pr-5 pl-16"
    >
      <h2 class="font-semibold mb-5">
        {{ faqs[Number(active) - 1].question }}
      </h2>
